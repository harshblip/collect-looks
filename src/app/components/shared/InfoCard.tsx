    ...
    <>
      <AnimatePresence>
        <motion.div
          ...
          className="absolute bg-black/20 top-0 bottom-0 w-full flex justify-center items-center -ml-12 font-product z-20"
        >
          <div className="bg-white overflow-auto rounded-md p-6 h-[40rem] w-[32rem] flex flex-col space-y-6">
            <div className="flex justify-center items-center flex-col">
              <button
                ...
                className="active:scale-95 hover border border-gray-400 rounded-md flex space-x-2 justify-center p-1 ml-[28.5rem] -mt-2 transition hover:shadow-md text-gray-600"
              >
              ...
              {file.file_type === "image" ? (
                ...
                <InfoBanner type={file.file_type} file_name={file.file_name} />
              )}
            </div>
            ...
            <div className="flex flex-col space-y-4">
              ...
              <Image
                src={
                  file.file_url
                    ? "https://cdn.dribbble.com/userupload/44201289/file/original-77d7237957eff0cc9adf154a8a9cc6e3.png?resize=2048x1423&vertical=center"
                    : ""
                }
                height={0}
                width={40}
            ...
            <hr className="text-primary border-[1.8px]" />

            <div className="flex flex-col space-y-4 rounded-md">
              <div className="flex items-center space-x-2">
                <LockClosedIcon className="w-5 text-secondary" />
                <p className="text-xl text-secondary"> Security Limitations </p>
              </div>
              <div className="bg-gray-100 rounded-md space-y-2 p-4">
                {!file.is_locked ? (
                  <div className="flex space-x-4 items-center">
                    <CubeTransparentIcon className="text-secondary w-8" />
                    <div className="flex flex-col text-secondary space-y-1 text-md">
                      <p>No limitations applied</p>
                      <p>If any are applied, they will appear here</p>
                    </div>{" "}
                  </div>
                ) : (
                  <div className="flex space-x-4 items-center">
                    <CubeIcon className="text-secondary w-8" />
                    <div className="flex flex-col text-secondary space-y-1">
                      <p>Password restricted</p>
                      <p>file is protected with password set by the owner</p>
                    </div>{" "}
                  </div>
                )}
              </div>
                      ...
                      <div className=" text-xl flex space-x-2 text-secondary font-medium">
                        {filePath.length > 0 ? (
                          filePath.map((x, i) => (
                            ...
                            >
                              {" "}
                              <div className="flex space-x-2 border border-gray-400 rounded-md p-2">
                                <FolderIcon
                                  width={18}
                          ...
                          <div className="flex space-x-2 items-center">
                            {" "}
                            <HomeIcon className="w-5 text-secondary" />
            ...
            <div className="flex justify-between mt-6">
              <div className="flex flex-col">
                ...
                <p className=" text-4xl text-gray-300 font-medium mt-1">
                  {" "}
              ...
              <div className="flex flex-col">
                <p className="flex justify-end text-sm text-primary">
                  {" "}
                  Opened{" "}
                </p>
                <p className=" text-4xl text-gray-300 font-medium mt-1">
                  {" "}
                  {file.updated_at.substring(0, 10)}{" "}
                </p>
              </div>
            </div>
          </div>
