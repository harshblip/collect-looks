    ...
    <div className="flex flex-col">
      ...
      >
        {uploading ? (
          ...
          <div
            className={`w-[32rem] ${files.length ? `max-h-[40rem]` : `h-[32rem]`} rounded-lg flex flex-col items-center p-6 justify-center text-center cursor-pointer transition-colors duration-300 ${isDragging ? "bg-gray-100 border-gray-400" : "border-gray-300 bg-white"}`}
          >
            <div
                ...
                <>
                  <div className="bg-[#fafaff] p-4 h-full w-full flex items-center justify-center flex-col rounded-md">
                    <div className="flex space-x-4 text-gray-600">
                      <PhotoIcon
                        className={`-rotate-20 ${files.length ? `w-4` : `w-6`}`}
                      />
                      <CameraIcon
                        className={`${files.length ? `w-4` : `w-8`} -mt-6`}
                      />
                      <FolderIcon
                        className={`rotate-20 ${files.length ? `w-4` : `w-6`}`}
                      />
                  ...
                  <div className="bg-[#fafaff] shadow-md p-4 h-full w-full flex items-center justify-center flex-col rounded-md">
                    ...
                    {!files.length && (
                      <p className="text-sm text-gray-400 mt-4">
                        {" "}
                        supports all general image, video, <br /> audio and doc
                        formats{" "}
                      </p>
                    )}
                  </div>
                          ...
                          <div className="flex space-x-4 items-center">
                            <div className="w-6 text-secondary ml-2">
                              {getFileCategory(file.type) === "image" ? (
                                <PhotoIcon />
                              ) : getFileCategory(file.type) === "video" ? (
                                <PlayIcon />
                            ...
                            <div className="flex flex-col">
                              <span className="text-md truncate max-w-xs">
                                {file.name}
                              </span>
