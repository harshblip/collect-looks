...
export default function Trash() {
    ...
    const { mutate: enableAutoDelete, data: enabledOrNot } = useEnableAutoDelete()
    ...
    return (
        <>
            <div className="flex flex-col space-y-0 mt-4 p-8 font-product">
                {
                    data ? <> <div className="flex justify-between w-[100%]">
                        <p className="text-4xl fixed font-medium h-16 pt-12 -mt-12 text-secondary bg-white"> Trashed </p>
                        <div className="flex space-x-4 items-center absolute right-20">
                            <p className="text-gray-400 text-sm">enable auto delete</p>
                            <Checkbox
                                onClick={() => enableAutoDelete({ checked: checked, userId: 3 })}
